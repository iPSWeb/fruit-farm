...
class PSCoinApi {
	...
	/* Make the request with to the NXT server
	...
	protected function _request(){
		...
		if(is_array($this->aInput)) {
			...
			switch($this->protocol) {
				case 'https':
				case 'http':
					$this->sCmd = $sTimeOut.' curl -sk --data "'.$sQuery.'" '.$this->protocol.'://'. $this->host .':'.$this->nxt_port.'/nxt';
					break;
				case 'ssh':
    ...
    */ 
	function getResponse($attribute='') {
		...
		if(empty($sResp)) {
			...
			$this->oResponse->errorDescription='PSCoin node Timeout';
