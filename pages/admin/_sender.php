...
if(isset($_POST['title'])){
    ...
    if(strlen($title) > 3){
        if(strlen($mess) > 10){
            $result=$pdo->prepare("INSERT INTO db_sender (name, mess, date_add) VALUES (:title,:mess,:time)");
            ...
            echo '<center><b>Сообщение должно быть больше 10 символов</b></center><BR />';
        }
    }else{
        echo '<center><b>Заголовок должен быть больше 3х символов</b></center><BR />';
    }
}
# Добавление рассылки
if(isset($_GET['add'])){
...
# Удаление
if(isset($_POST['del'])){
    $result=$pdo->prepare("DELETE FROM db_sender WHERE id = :id");
    $result->execute(array('id'=>intval($_POST['del'])));
    echo '<center><b>Рассылка удалена</b></center><BR />';
}
$result=$pdo->query("SELECT * FROM db_sender ORDER BY id DESC");
