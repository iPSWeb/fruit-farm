...
if(isset($_GET['last_31'])){
	...
	$result=$pdo->query("SELECT * FROM db_payment WHERE date_add > $dlim ORDER BY id DESC");
	$days_money = array();
	$days_insert = array();
...
if(isset($_GET['list_day'])){
	$result=$pdo->query("SELECT * FROM db_payment ORDER BY id DESC");
	$days_money = array();
	$days_insert = array();
	if($result->rowCount() > 0){
...
$lim = $num_p * 100;
$result=$pdo->query("SELECT * FROM db_payment ORDER BY id DESC LIMIT {$lim}, 100");
...
<?PHP
	...
	?>
	<tr class="htt">
	<td align="center"><a href="/admin/users/edit/<?=$data['user_id']; ?>" class="stn"><?=$data['user']; ?></a></td>
...
$result=$pdo->query("SELECT COUNT(*) FROM db_payment");
$all_pages = $result->fetchColumn();
	if($all_pages > 100){
	$sort_b = (isset($_GET["sort"])) ? intval($_GET["sort"]) : 0;
	$nav = new navigator;
	$page = (isset($_GET["page"]) AND intval($_GET["page"]) < 1000 AND intval($_GET["page"]) >= 1) ? (intval($_GET["page"])) : 1;
	echo "<BR /><center>".$nav->Navigation(10, $page, ceil($all_pages / 100), "/admin/payments/page/"), "</center>";
	}
?>
</div><div class='clr'></div>
